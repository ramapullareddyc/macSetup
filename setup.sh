...
phase_1_zsh() {
  [[ "$(basename "$SHELL")" == "zsh" ]] && echo "✅ Default shell already zsh" || chsh -s /bin/zsh 2>/dev/null || true
}
...
phase_2() {
  ...
  # 2.6 Generate .zshrc (backup existing if present)
  if [[ -f ~/.zshrc ]]; then
...
phase_5_gemini_cli() {
  ...
  command -v gemini &>/dev/null && echo "✅ Gemini CLI already installed" || npm install -g @google/gemini-cli
}
...
phase_6_android_sdk() {
  ...
  echo "no" | avdmanager create avd -n "Pixel_8_API_36" \
    ...
    -d "pixel_8" --force 2>/dev/null || echo "⚠️  AVD creation failed — create manually in Android Studio"
}
